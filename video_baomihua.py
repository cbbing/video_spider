...
class BaomihuaVideo(BaseVideo):
    def __init__(self):
        BaseVideo.__init__(self)
        self.engine = '爆米花'
        self.site = 'baomihua'
        #self.album_url = 'http://so.56.com/so/q_key' #专辑的url
        self.general_url = 'http://so.baomihua.com/DataList.aspx?keyword=keys&seachtype=0&createtime=0&playcost=tid&page=pid' #普通搜索的url
        self.filePath = 'baomihua_video'
    ...
    def search(self, key):
        ...
        for lengthtype in lengthtypes:
            ...
            for i in range(self.pagecount):
                url = self.general_url.replace('tid', lengthtype).replace('pid', str(i+1)).replace('keys',key)
    ...
    def parse_data(self, text, page, legth_type, key):
        ...
        #视频链接-全部结果
        tableArea = soup.find('div', {'class':'row clearfix'})
        if not tableArea:
            return []

        dramaList = tableArea.findAll('li', attrs={'class':'vimg'})
        for drama in dramaList:
